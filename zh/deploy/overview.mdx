...
Chainlit 应用可以通过多个平台使用。一次编写助手逻辑，随处使用！
...
在生产环境运行 Chainlit 应用时，应始终在
`chainlit run` 命令中添加 `-h`。否则会在服务端打开浏览器窗口，可能导致部署失败。

### 仔细检查 host

默认情况下，Chainlit 服务器 host 为 `127.0.0.1`。
通常，如果你在 docker 中运行 Chainlit，需要在 chainlit 命令中添加 `--host 0.0.0.0`。

### 考虑 WebSocket
Chainlit 基于 WebSocket 构建，这意味着你部署应用的服务必须支持 WebSocket。在自动扩缩容时，请确保启用粘性会话（或会话亲和性）。

即使启用了粘性会话，负载均衡器有时仍难以将客户端一致地路由到同一容器。
在这种情况下，你可以在 `.chainlit/config.toml` 文件中设置 `transports = ["websocket"]`。

### 在子路径上部署 Chainlit
如果你需要将 Chainlit 应用部署到子路径，例如
`https://my-app.com/chainlit`，则需要在运行 `chainlit run` 命令时设置 `--root-path
/chainlit` 标志。这将确保应用从正确的路径提供服务。

### 跨域
如果最终用户从与服务器相同的源访问 Chainlit UI，一切将开箱即用。
但是，如果你在网站上嵌入 Chainlit，由于 CORS，连接将失败。

在这种情况下，你需要更新 `.chainlit/config.toml` 的 `allow_origins` 字段。

## 社区资源

在本地成功设置并测试 Chainlit 应用后，下一步是通过部署到托管服务，让更广泛的受众可以访问它。本指南提供了多种自托管 Chainlit 应用的选项。

- [Ploomber Cloud](https://docs.cloud.ploomber.io/en/latest/apps/chainlit.html)
- [AWS](https://ankushgarg.super.site/how-to-deploy-your-chatgpt-like-app-with-chainlit-and-aws-ecs)
- [Azure Container](https://techcommunity.microsoft.com/t5/fasttrack-for-azure/create-an-azure-openai-langchain-chromadb-and-chainlit-chat-app/ba-p/3885602)
- [Google Cloud Run](https://pseudohvr.medium.com/deploying-chainlit-on-gcp-72231ba6b77f)
- [Google App Engine](https://github.com/amjadraza/langchain-chainlit-docker-deployment-template)
- [Replit](https://replit.com/@DanConstantini/Build-a-Chatbot-with-OpenAI-LangChain-and-Chainlit?v=1)
- [Render](https://discord.com/channels/1088038867602526210/1126834266504966294/1126845898287230977)
- [Fly.io](https://dev.to/willydouhard/how-to-deploy-your-chainlit-app-to-flyio-38ja)
- [HuggingFace Spaces](https://github.com/Chainlit/cookbook/tree/main/chroma-qa-chat)
